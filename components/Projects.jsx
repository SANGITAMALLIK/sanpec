...
// ---------- Reusable Tab Button ----------
const TabButton = ({ label, sublabel, isActive, onClick }) => (
  <div className="relative md:pl-16">
    <div className="hidden md:block absolute left-4 top-8 w-4 h-4 bg-gray-50 border-2 border-gray-400 rounded-full" />
    <div
      className={`hidden md:block absolute left-7 top-9 h-0.5 w-9 ${
    ...
    >
      <h3 className="text-xl md:text-2xl font-black text-gray-900">{label}</h3>
      <p className="text-xs md:text-sm text-gray-600 mt-1 md:mt-2">{sublabel}</p>
      {isActive && (
        <div className="absolute right-3 md:right-4 top-1/2 -translate-y-1/2">
          <div className="w-2.5 md:w-3 h-2.5 md:h-3 bg-[#CD091B] rounded-full animate-pulse" />
        </div>
      )}
    </button>
  </div>
...
// ---------- Project / Research Card (used on desktop & mobile) ----------
const ProjectCard = ({ item, activeTab }) => (
  <Link
    href={item.url}
    ...
    <div className="relative h-full overflow-hidden rounded-xl shadow-lg border-2 border-white/50 group-hover:scale-105 group-hover:shadow-2xl transition-all duration-500">
          ...
          {activeTab === "projects" ? (
            <>
              <h3
                ...
                style={{
                  textShadow:
                    "0 4px 30px rgba(0,0,0,0.9), 0 2px 10px rgba(0,0,0,0.9)",
                }}
                dangerouslySetInnerHTML={{ __html: item.title }}
              />
              <div className="flex items-center justify-center gap-2 mt-2 md:mt-3">
                <div className="h-px w-8 md:w-12 bg-[#CD091B]" />
                <div className="w-1.5 md:w-2 h-1.5 md:h-2 bg-[#CD091B] rounded-full" />
                <div className="h-px w-8 md:w-12 bg-[#CD091B]" />
              </div>
            </>
          ) : (
              ...
              {item.excerpt && (
                <>
                  <div className="flex items-center justify-center gap-2">
                    <div className="h-px w-8 md:w-10 bg-white/50" />
                    <div className="w-1 md:w-1.5 h-1 md:h-1.5 bg-white/50 rounded-full" />
                    <div className="h-px w-8 md:w-10 bg-white/50" />
                  </div>
                  <div
      ...
      <div className="absolute bottom-4 md:bottom-5 right-4 md:right-5 opacity-0 group-hover:opacity-100 scale-75 group-hover:scale-100 transition-all duration-500">
        <div className="bg-white p-2 md:p-3 rounded-full shadow-2xl group-hover:bg-[#CD091B] transition-colors">
          <svg
            className="w-4 md:w-5 h-4 md:h-5 text-[#CD091B] group-hover:text-white transition-colors"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
...
// ---------- Why SANPEC Card (special design) ----------
const WhySanpecCard = ({ item }) => (
    ...
    <div className="relative overflow-hidden rounded-2xl bg-white shadow-lg border border-gray-200 group-hover:shadow-2xl transition-all duration-500 h-full">
      ...
      <div className="p-6 md:p-8 space-y-4 bg-white">
        ...
        <div className="flex items-center gap-3">
          <div className="h-1 w-12 bg-gradient-to-r from-[#CD091B] to-transparent rounded-full" />
          <div className="w-2 h-2 bg-[#CD091B] rounded-full" />
        </div>
        <div
      ...
      <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-[#CD091B]/10 to-transparent rounded-bl-[100px] opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
      <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-[#CD091B] to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-500" />
    </div>
  </Link>
);
...
const EngineeringTabsSlider = () => {
  ...
  const whySanpecData = [
    ...
    },
    {
      ...
      excerpt:
        "Ajay Mallik, P.E. President and CEO of SANPEC Engineering, shares his team's critical role...",
      category: "Pattern",
    ...
    },
    {
      ...
      title: "365 Approach",
      excerpt:
        "At SANPEC, we embrace a '365' approach, three strategic directions, six...",
      category: "Excellence",
    ...
    },
    {
      ...
      excerpt:
        "As a leading transmission and distribution & engineering company, SANPEC is committed to engineering...",
      category: "Pillars",
    ...
    const fetchData = async () => {
      ...
      try {
        const [projectsRes, researchRes] = await Promise.all([
          fetch(
            "https://news.sanpec-excellence.com/wp-json/wp/v2/posts?categories=42&per_page=100&_embed"
          ),
          fetch(
        ...
        const formattedProjects = projectsData.map((post) => {
          ...
          let categoryName =
            categories.length > 0 ? categories[0].name : "Project";
        ...
        const formattedResearch = researchData.map((post) => {
          const categories = post._embedded?.["wp:term"]?.[0] || [];
          let categoryName =
            categories.length > 0 ? categories[0].name : "Research";
          const tempDiv = document.createElement("div");
          tempDiv.innerHTML = categoryName;
          categoryName = tempDiv.textContent || tempDiv.innerText || categoryName;
    ...
    <div className="w-full py-8 md:py-12 px-4 sm:px-6 lg:px-8 bg-gray-50">
      ...
      <div className="max-w-[1600px] mx-auto mb-8 md:mb-12">
        <div className="flex items-center justify-center gap-3">
          <div className="h-[2px] w-20 sm:w-32 md:w-48 bg-gradient-to-r from-transparent to-[#CD091B]" />
          <div className="relative">
            <svg
              className="w-8 h-8 md:w-10 md:h-10 text-[#CD091B]"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div className="h-[2px] w-20 sm:w-32 md:w-48 bg-gradient-to-l from-transparent to-[#CD091B]" />
        </div>
      </div>
          ...
          <div className="w-full lg:w-80 flex-shrink-0">
            <div className="lg:sticky lg:top-8">
              <div className="relative space-y-4 md:space-y-6 pb-12 md:pb-16">
                ...
                <div className="hidden md:block absolute left-6 top-0 bottom-12 w-1 bg-gradient-to-b from-gray-300 via-gray-400 to-gray-300" />

                <TabButton
                  label="Our Projects"
                  sublabel="Transmission and Distribution"
                  isActive={activeTab === "projects"}
                  onClick={() => setActiveTab("projects")}
                />
                <TabButton
                  label="Research & Innovation"
                  sublabel="Technical Papers"
                  isActive={activeTab === "research"}
                  onClick={() => setActiveTab("research")}
                />
                <TabButton
                  label="Why SANPEC"
                  sublabel="Building the Grid of the Future"
                  isActive={activeTab === "whysanpec"}
                  onClick={() => setActiveTab("whysanpec")}
                />

                {/* Tower base (desktop) */}
                <div
                  className="hidden md:block absolute left-2 bottom-0 w-9 h-12 bg-gradient-to-b from-gray-400 to-gray-500 opacity-30"
                  style={{
                    clipPath: "polygon(32% 0%, 70% 0%, 100% 100%, 0% 100%)",
                  }}
                />
              </div>
            </div>
              ...
              {loading ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                    <div
                      key={i}
                      className="h-64 md:h-80 bg-gray-200 animate-pulse rounded-xl"
                    />
                  ))}
                </div>
              ) : (
                <>
                  ...
                  >
                    {activeTab === "whysanpec" ? (
                      // Why SANPEC – all items, no pagination
                      currentData.map((item) => (
                        <WhySanpecCard key={item.id} item={item} />
                      ))
                    ) : (
                      <>
                        {/* Desktop – show all */}
                  ...
                  {/* Pagination – only mobile, not for Why SANPEC */}
      ...
      {/* styled-jsx (kept exactly as before) */}
      <style jsx>{`
