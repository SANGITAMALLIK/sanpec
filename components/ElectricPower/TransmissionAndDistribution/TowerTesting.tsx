...
export default function TowerTesting() {
      ...
      <div className="bg-gradient-to-br from-gray-50 to-white rounded-xl p-4 border border-gray-300 shadow-sm">
        <div className="mb-4 pb-3 border-b border-gray-300">
          <div className="flex items-center gap-2 text-[#CD091B]">
            ...
            <span className="text-xs font-semibold uppercase tracking-wider">Tower Testing</span>
          </div>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {towerTestingSubTabs.map((tab) => {
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {content.images.map((img, idx) => (
                  <div key={idx} className={`overflow-hidden rounded-lg border-2 border-gray-200 hover:border-[#CD091B] transition-all duration-300 shadow-sm hover:shadow-md group ${idx === 0 ? 'col-span-2' : ''}`}>
                    <img src={img} alt={`${content.title} ${idx + 1}`} className="w-full h-auto object-cover group-hover:scale-105 transition-transform duration-500" />
                  </div>
                ))}
              </div>
            </div>
